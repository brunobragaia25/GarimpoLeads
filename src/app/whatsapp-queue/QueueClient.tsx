...
function Kbd({ children }: { children: React.ReactNode }) {
  return (
    <kbd className="rounded border border-current/30 px-1.5 py-0.5 font-mono text-[10px] opacity-70">
      {children}
    </kbd>
  );
}
...
export function QueueClient({ leads, total }: { leads: QueueLead[]; total: number }) {
  ...
  const [lastDone, setLastDone] = useState<{ lead: QueueLead; index: number } | null>(null);
        ...
        <article className="rounded-xl border border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-950">
          ...
          <div className="p-6">
            <div className="mb-2 flex items-center justify-between">
              ...
              <button
                onClick={copyMessage}
                className="inline-flex items-center gap-1.5 rounded-md px-2 py-1 text-xs font-medium text-zinc-500 hover:bg-zinc-100 hover:text-zinc-800 dark:hover:bg-zinc-900 dark:hover:text-zinc-200"
              >
                {copied ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}
                {copied ? "Copiada!" : "Copiar"}
              </button>
            </div>
            <div className="max-w-xl rounded-2xl rounded-tl-sm bg-[#dcf8c6] px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap text-zinc-900 dark:bg-[#005c4b] dark:text-zinc-100">
              {current.message}
            </div>
          </div>
